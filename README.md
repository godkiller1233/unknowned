...
For multiple web servers, deploy each instance with the same `DATABASE_URL` and the same `JWT_SECRET`.

Voice and video calls connect peer-to-peer (WebRTC mesh). Users behind symmetric NAT need a TURN relay. Set it once in the **Owner Console → Voice relay (TURN) settings**: the config is stored in the shared PostgreSQL database, so every server instance serves the same relay immediately — no environment edits or restarts, ideal for multi-instance deployments (the admin endpoint `GET/PUT /api/admin/rtc` backs it; the credential is write-only and never returned). The environment variables `TURN_URLS`, `TURN_USERNAME`, `TURN_CREDENTIAL` are only the fallback / initial value, used when the database value is empty. The server publishes the list in `/api/bootstrap` and every peer connection — DM calls and room/channel meshes — uses it. Without TURN, clients fall back to public STUN, which covers most connections but not symmetric NAT; `/api/health/rtc` and the Owner Console badge report when no relay is reachable. Run your own TURN server (e.g. coturn) or use a managed provider. Accounts and application records are shared through PostgreSQL, and Socket.IO broadcasts are synchronized through PostgreSQL pub/sub. The client uses WebSocket-only transport, so sticky sessions are not required. Set `numInstances` to 2 or more only on a Render plan that supports horizontal scaling.
